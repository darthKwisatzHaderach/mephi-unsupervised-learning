"""Узкая сетка mordred_w ∈ {0.55, 0.58, 0.60, 0.62} vs ref w55 (public 272.99)."""
import time
...
def main() -> None:
    ...
    print("ref mord_w55:", {s: round(ref_scores[s], 2) for s in SEEDS})
    t0 = time.perf_counter()
    rows = []

    for mw in [0.55, 0.58, 0.60, 0.62]:
        for seed in SEEDS:
    ...
    sm.to_csv("phase_g_mord_tune4.csv", index=False)
