...
Запуск:
  python make_submission_phase_v.py cc50_knn_only
"""
...
from run_phase_v import CANDIDATES, fit_phase_v
...
def build_test(variant: str, seed: int = 42) -> np.ndarray:
    ...
    clear_fold_cache()
    _, test = fit_phase_v(X, Xt, y, ext, blocks, seed, cfg)
    return test
...
def main() -> None:
    ...
    out = f"submission_phase_v_{args.variant}.csv"
