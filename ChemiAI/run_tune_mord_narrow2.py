"""Узкая сетка mordred_w ∈ {0.40, 0.42, 0.45, 0.48} vs ref w40 (public 273.28)."""
import time
...
def main() -> None:
    ...
    print("ref mord_w40:", {s: round(ref_scores[s], 2) for s in SEEDS})
    t0 = time.perf_counter()
    rows = []

    for mw in [0.40, 0.42, 0.45, 0.48]:
        for seed in SEEDS:
    ...
    print("\n=== mordred high-weight grid ===")
    print(sm.to_string(index=False))
    sm.to_csv("phase_g_mord_tune2.csv", index=False)
